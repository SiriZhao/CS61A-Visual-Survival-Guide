# CS61A-Visual-Survival-Guide
🐻 UCB CS61A Ultimate Interactive Web-based Visual Notes | 终极互动网页版可视化笔记。 (updating). 
...
def horse(mask):
    ...
    def mask(horse):          # 2. Local 'mask' shadows the parameter / 局部 mask 覆盖了参数名
        return horse
    return horse(mask)        # 3. 'horse' here is still the external lambda / 这里的 horse 依旧是外部 lambda！
